...
int main(int argc, char *argv[])
{
	...
	unitMeasurement unitArray [arraySize];
	...
	else
	{
		...
		// fill array with unitMeasurment struct objects
		PopulateArraysFromFile(unitArray, fileName);
		...
		while (promptUser)
		{
			...
			if (usrInputOne.length() != 0) // check if the user wants to quit otherwise goto second prompt
			{
				...
				{
					if(UnitCompatible(unitArray, usrInputOne, usrInputTwo)) // true if the unit types match
					{
